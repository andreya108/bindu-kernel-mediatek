...
static DEFINE_MUTEX(sn3193_mutex);
/****************************************************************************
* defined
***************************************************************************/
...
int sn3193_write_reg_(u8 addr, u8 para)
{
	return sn3193_write_reg(addr, para);
}
...
#define SN3193_RED_MASK    	0xff0000
#define SN3193_GREEN_MASK	0x00ff00
#define SN3193_BLUE_MASK	0x0000ff
#define SN3193_CYAN_MASK	0x00ffff
#define SN3193_PURPLE_MASK	0xff00ff
#define SN3193_YELLOW_MASK	0xffff00

	void sn3193_led_set_color(struct led_classdev *led_cdev, enum led_brightness value)
	{
		int mask = 0;

		int color_value = (value<<SN3193_BL_R_OFFSET_BIT)+(value<<SN3193_BL_G_OFFSET_BIT)+(value<<SN3193_BL_B_OFFSET_BIT);

		struct sn3193_leds_priv *led_data =
		container_of(led_cdev, struct sn3193_leds_priv, cdev);

		//	if (!strcmp(led_data->name, "white")); skip
		if (!strcmp(led_cdev->name, "red")) color_value = color_value & SN3193_RED_MASK;
		if (!strcmp(led_cdev->name, "green")) color_value = color_value & SN3193_GREEN_MASK;
		if (!strcmp(led_cdev->name, "blue")) color_value = color_value & SN3193_BLUE_MASK;
		if (!strcmp(led_cdev->name, "cyan")) color_value = color_value & SN3193_CYAN_MASK;
		if (!strcmp(led_cdev->name, "purple")) color_value = color_value & SN3193_PURPLE_MASK;
		if (!strcmp(led_cdev->name, "yellow")) color_value = color_value & SN3193_YELLOW_MASK;

		LEDS_DEBUG("[LED %s] value=0x%x -> 0x%06x\n", led_cdev->name, value, color_value);

		sn3193_led_set(led_cdev, color_value);
	}
	...
	struct sn3193_leds_priv *g_sn3193_leds_data[9];
	...
	static int sn3193_leds_setup_color(struct platform_device *pdev, char *name, int num)
	{
		int ret=0;

		g_sn3193_leds_data[num] = kzalloc(sizeof(struct sn3193_leds_priv), GFP_KERNEL);
		if (!g_sn3193_leds_data[num]) {
			ret = -ENOMEM;
			return ret;
		}

		g_sn3193_leds_data[num]->cdev.name = name;
		g_sn3193_leds_data[num]->cdev.brightness_set = sn3193_led_set_color;
		g_sn3193_leds_data[num]->cdev.max_brightness = 0xff;
		g_sn3193_leds_data[num]->cdev.blink_set = sn3193_blink_set;
		INIT_WORK(&g_sn3193_leds_data[num]->work, sn3193_led_work);
		g_sn3193_leds_data[num]->gpio = GPIO_LED_EN;
		g_sn3193_leds_data[num]->level = 0;

		return led_classdev_register(&pdev->dev, &g_sn3193_leds_data[num]->cdev);
	}
	...
	static int __init sn3193_leds_platform_probe(struct platform_device *pdev)
	{
		...
		ret = sn3193_leds_setup_color(pdev, "red",    2); if (ret) goto err;
		ret = sn3193_leds_setup_color(pdev, "green",  3); if (ret) goto err;
		ret = sn3193_leds_setup_color(pdev, "blue",   4); if (ret) goto err;
		ret = sn3193_leds_setup_color(pdev, "cyan",   5); if (ret) goto err;
		ret = sn3193_leds_setup_color(pdev, "purple", 6); if (ret) goto err;
		ret = sn3193_leds_setup_color(pdev, "yellow", 7); if (ret) goto err;
		ret = sn3193_leds_setup_color(pdev, "white",  8); if (ret) goto err;
	...
	************************************************************************************/
	...
	MODULE_AUTHOR("jixu@lenovo.com, andreya108@4pda/xda");
