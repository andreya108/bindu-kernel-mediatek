...
static cust_set_brightness blink_set_brightness;
#endif
...
int mt_led_set_pwm(int pwm_num, struct nled_setting* led)
{
    ...
    pwm_setting.mode = PWM_MODE_OLD;
        ...
        LEDS_DEBUG("[LEDS_MT6589] led_set_pwm: mode=%d,pwm_no=%d\n", led->nled_mode, pwm_num);
...
static int led_breath_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting* led)
{
	//int time_index = 0;
	//int duty = 0;
	LEDS_DEBUG("[LEDS_MT6589] led_blink_pmic: pmic_type=%d\n", pmic_type);
...
int mt_backlight_set_pwm(int pwm_num, u32 level, u32 div, struct PWM_config *config_data)
{
	...
	else
	{
		...
		LEDS_DEBUG("[LEDS_MT6589] backlight_set_pwm:duty is %d\n", level);
		LEDS_DEBUG("[LEDS_MT6589] backlight_set_pwm:clk_src/div/high/low is %d%d%d%d\n", pwm_setting.clk_src,pwm_setting.clk_div,pwm_setting.PWM_MODE_FIFO_REGS.HDURATION,pwm_setting.PWM_MODE_FIFO_REGS.LDURATION);
		if(level>0 && level <= 32)
		{
			pwm_setting.PWM_MODE_FIFO_REGS.GUARD_VALUE = 0;
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
		...
		LEDS_DEBUG("[LEDS_MT6589] PMIC#%d:%d\n", pmic_type, level);
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK0)
		{
			...
			//if(led_init_flag[0] == false)
			{
				...
				#if 1//def CONFIG_LENOVO//lenovo jixu modify led current as 4mA 100%duty
	...
	*/
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_GPIO:
			printk("[LEDS_MT6589] brightness_set_cust:go GPIO mode! level=%d\n",level);
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	if (led_data->level != level) {
		...
		{
				//LEDS_DEBUG("[LEDS_MT6589] Set Backlight directly %d at time %lu\n",led_data->level,jiffies);
			     ...
			     unsigned long *delay_off)
{
	...
	LEDS_DEBUG("[LEDS_MT6589] [mt_mt65xx_blink_set] wl=%d, delay_on=%d, delay_off=%d\n", got_wake_lock, *delay_on, *delay_off );
	...
	if (*delay_on != led_data->delay_on || *delay_off != led_data->delay_off) {
		...
		if (led_data->delay_on && led_data->delay_off) { // enable blink
            ...
            LEDS_DEBUG("[LEDS_MT6589] [mt_mt65xx_blink_set] (%s) -> enable blink!\n",led_data->cust.name);
#ifdef LEDS_GPIO_SLEEP_WORKAROUND
            if(led_data->cust.mode == MT65XX_LED_MODE_GPIO)
                blink_set_brightness = (cust_set_brightness)(led_data->cust.data);
#endif
...
//					printk("[LEDS_MT6589] [mt_mt65xx_blink_set] wake_lock 1!\n");
					wake_lock(&leds_suspend_lock);
				}
			}
			else if (!got_wake_lock) {
//				LEDS_DEBUG("[LEDS_MT6589] [mt_mt65xx_blink_set] wake_lock 2!\n");
//				wake_lock(&leds_suspend_lock);
				got_wake_lock = 1;
			}
		}
...
//					printk("[LEDS_MT6589] [mt_mt65xx_blink_set] wake_unlock 1!\n");
				}
			}
			else if (got_wake_lock) {
//			wake_unlock(&leds_suspend_lock);
				got_wake_lock = 0;
//				LEDS_DEBUG("[LEDS_MT6589] [mt_mt65xx_blink_set] wake_unlock 2!\n");
			}
		}
		return -1;
	}
...
#ifdef LEDS_GPIO_SLEEP_WORKAROUND
void mt_leds_suspend()
{
    LEDS_DEBUG("[LEDS_MT6589] %s\n", __func__);
    if (blink_set_brightness != NULL) {

        LEDS_DEBUG("[LEDS_MT6589] suspending while in GPIO blinking mode, turning LEN on!\n");
        blink_set_brightness(255);
    }
}
