...
*****************************************************************/
#if 1
#define MSG(fmt,args...) printk(fmt,##args)

#define MSG_FUNC_ENTRY(f)    MSG("<PMAPI FUNC>: %s\n", __FUNCTION__)
#else
...
bool hwPowerOn(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt, char *mode_name)
{
    ...
    if(first_power_on_flag == 1)
    {
        ...
        MSG("[hwPowerOn] init done.\r\n");
    }

#if 1
    if(powerId >= MT65XX_POWER_COUNT_END)
    {
        MSG("[MT65XX PMU] Error!! powerId is wrong\r\n");
        return FALSE;
    }
    for (i = 0; i< MAX_DEVICE; i++)
    {
        ...
        {
            MSG("[%s] acquire powerId:%d index:%d mod_name: %s\r\n",  __FUNCTION__,powerId, i, mode_name);
    ...
    {
        MSG("[hwPowerOn] g_MT_PMIC_BusHW.Power[powerId].dwPowerCount (%d) > 1\r\n", g_MT_PMIC_BusHW.Power[powerId].dwPowerCount);
        return TRUE;
    }
#endif
...
bool hwPowerDown(MT65XX_POWER powerId, char *mode_name)
{
    ...
    if(powerId >= MT65XX_POWER_COUNT_END)
    {
        MSG("%s:%s:%d powerId:%d is wrong\r\n",__FILE__,__FUNCTION__, 
            __LINE__ , powerId);
        return FALSE;
    }    
    if(g_MT_PMIC_BusHW.Power[powerId].dwPowerCount == 0)
    {
        MSG("%s:%s:%d powerId:%d (g_MT_PMIC_BusHW.dwPowerCount[powerId] = 0)\r\n", 
    ...
    for (i = 0; i< MAX_DEVICE; i++)
    {
        ...
        {
            MSG("[%s] powerId:%d index:%d mod_name: %s\r\n",  __FUNCTION__,powerId, i, mode_name);
    ...
    if(!bFind)
    {
        MSG("[%s] Cannot find [%d] master is [%s]\r\n",__FUNCTION__,powerId, mode_name);        
        return TRUE;
    }
    ...
    if(g_MT_PMIC_BusHW.Power[powerId].dwPowerCount > 0)
    {
        MSG("[hwPowerDown] g_MT_PMIC_BusHW.Power[powerId].dwPowerCount (%d) > 0\r\n", g_MT_PMIC_BusHW.Power[powerId].dwPowerCount);
        return TRUE;
    }
#endif
    /* Turn off PMU LDO*/
    //MSG(CG,"[%d] PMU LDO Disable\r\n",powerId );
    MSG("[hwPowerDown] disable %d by %s \r\n", powerId, mode_name);
