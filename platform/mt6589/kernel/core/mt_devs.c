...
static struct platform_device actuator_dev_9718 = {
...
#elif defined(LENOVO_SN3193_LED_SUPPORT_NEW)//lenovo add by jixu@lenovo.com
static struct platform_device sn3193_leds_device = {
	.name = "leds-sn3193",
	.id = -1
};
#elif defined(LENOVO_SHINELED_SUPPORT)//lenovo add by andreya108@4pda/xda
static struct platform_device sn3193_leds_device = {
	.name = "SN3193",
	.id = -1
};
...
"\n",   bl_fb.base, bl_fb.size, FB_START, FB_SIZE);
	...
	if (retval != 0)
		return retval;
	printk("[JX]:device sn3193_new leds register\n");
#elif defined(LENOVO_SHINELED_SUPPORT)//lenovo add by andreya108@4pda/xda
	retval = platform_device_register(&sn3193_leds_device);
	if (retval != 0)
		return retval;
	printk("[JX]:device sn3193_shine leds register\n");
#endif
...
//#lenovo.sw wangsx3 20140515 add main backup sensor ov8865
    retval = platform_device_register(&actuator_dev_9718);
    if (retval != 0){
        printk("mytest DW9718  platform device registering..actuator_dev_9718 fail.\n");
        return retval;
    }
