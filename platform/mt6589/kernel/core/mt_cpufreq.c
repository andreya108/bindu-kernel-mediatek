...
#else
static unsigned int g_max_freq_on_suspend = DVFS_F1; /* default 1.2MHz */
#endif
...
{
    if (mt_cpufreq_pause) {
        dprintk("mt_cpufreq_keep_max_freq: mt_cpufreq_pause!\n");
        return 1;
    }
    ...
    if (g_ramp_down_count < RAMP_DOWN_TIMES) {
        dprintk("mt_cpufreq_keep_max_freq: %d < %d!\n",g_ramp_down_count,RAMP_DOWN_TIMES);
        return 1;
    }
...
//    #endif
    ...
    {
        dprintk("mt_cpufreq_keep_max_freq -> freqs.new = %d\n", freqs.new);
