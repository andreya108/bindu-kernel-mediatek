...
static unsigned int g_max_freq_on_suspend = DVFS_F1; /* default 1.2GHz */
#ifdef LENOVO_CPUFREQ_LIMIT

static int __init cpufreq_limit_setup(char *str)
{
    int tmp = g_max_freq_on_suspend;
    printk("OPT cpufreq.limit_on_suspend=%s\n", __func__, str);
    get_option(&str, &tmp);
    switch (tmp)
    {
        case DVFS_F1:
        ...
        case DVFS_F3:
        case DVFS_F4:
            g_max_freq_on_suspend = tmp;
            printk("OPT cpufreq.limit_on_suspend: set = %d\n", __func__, tmp);
            break;
        default:
            printk("OPT cpufreq.limit_on_suspend: wrong frequncy %d, ignoring\n", __func__, tmp);
    }
    return 0;
}
early_param("cpufreq.limit_on_suspend",cpufreq_limit_setup);
...
#ifdef LENOVO_CPUFREQ_LIMIT
/***************************
* show current cpufreq on suspend limit
****************************/
static int mt_cpufreq_limit_on_suspend_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    char *p = buf;
...
* set DVFS stauts by sysfs interface
*************************************/
static ssize_t mt_cpufreq_limit_on_suspend_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    unsigned int newfreq = 0;

    sscanf(buffer, "%d", &newfreq);
    ...
    switch (newfreq) {
        ...
        case DVFS_F3:
        case DVFS_F4:
            xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "set limit_on_suspend = %d\n", newfreq);
            g_max_freq_on_suspend = newfreq;
            break;
        default:
            xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "limit_on_suspend bad argument!! argument should be from set of allowed freqs!\n");
            break;
    }
