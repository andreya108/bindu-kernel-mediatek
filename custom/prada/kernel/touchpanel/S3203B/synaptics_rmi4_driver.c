...
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO("S3203", 0x38)};
...
static int tpd_rmi4_read_pdt(struct tpd_data *ts)
{
    ...
    for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
        ...
        dev_dbg(&ts->client->dev, "%s: page_number = %d\n", __func__, page_number);

        for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
                pdt_entry_addr -= PDT_ENTRY_SIZE) {
            pdt_entry_addr |= (page_number << 8);

            dev_dbg(&ts->client->dev, "%s: probing pdt_entry_addr = 0x%0x\n", __func__, pdt_entry_addr);
            ...
            retval = tpd_i2c_read_data(ts->client,
                    ...
                    sizeof(rmi_fd));
            if (retval <= 0) {
                dev_dbg(&ts->client->dev, "%s: tpd_i2c_read_data() = 0x%0x\n", __func__, retval);
...
/*
    ...
    dev_dbg(&ts->client->dev,
                    "%s: tpd product_id_string: %s\n",
                    __func__, &f01_query[11]);
*/
#if 0
flash_prog_mode:
...
{    u16 temp=0;
    ...
    int point_count;
    int ret=0;
    TPD_DMESG("enter tpd_process() ->read data to clear interrupt!!\n");
...
static int tpd_sw_power(struct i2c_client *client, int on)
{
    ...
    dev_dbg(&client->dev, "switch to %d\n", on);
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    struct synaptics_rmi4_device_info *rmi;
    dev_dbg(&client->dev, "enter tpd_probe()\n");
//    TPD_DMESG("%s:enter \n",__func__);
