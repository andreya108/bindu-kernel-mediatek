...
//while (1);
    ...
    if (fwu->firmware_data) {
        ...
        if (retval < 0)
            return retval;
        TPD_DMESG("%s: Firmware programmed\n", __func__);
    }
    ...
    if (fwu->config_data) {
        ...
        if (retval < 0)
            return retval;
        TPD_DMESG("%s: Configuration programmed\n", __func__);
    }
...
static int fwu_start_write_config(void)
{
    ...
    TPD_DMESG("%s: Start of write config process\n", __func__);
    ...
    TPD_DMESG("%s: End of write config process\n", __func__);
