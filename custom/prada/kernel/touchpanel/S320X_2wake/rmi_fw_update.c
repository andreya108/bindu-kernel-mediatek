...
static int fwu_read_blocks(unsigned char *block_ptr, unsigned short block_cnt, unsigned char command);

static void dump_configs(void);
static void dump_config(unsigned short cfg);
static void dump_buff(char *id, unsigned char *buf, int size);
static void log_config(void);

static unsigned char fw_store[47360];
...
static int fwu_check_version(void)
{
	...
	CTP_DBG("fwu_check_version: enter\n");
...
//while (1);
	...
	if (fwu->config_data) {
		...
		if (retval < 0)
			return retval;
		CTP_DBG("%s: Configuration programmed\n", __func__);
	}
...
//#define READ_NOT_WRITE
	...
	//save
	img_size = fwu->image_size;
	// dump fw?
	CTP_DBG("%s: TRYING DUMP FW via CMD_READ_CONFIG_BLOCK\n", __func__);
	retval = fwu_read_blocks(fw_store, sizeof(fw_store)/fwu->block_size, CMD_READ_CONFIG_BLOCK);
	if (fwu->image_size > 0) {
		CTP_DBG("%s: CMD_READ_CONFIG_BLOCK success, read %d bytes\n", __func__, fwu->image_size);
		dump_buff("FW_CMD_READ_CONFIG_BLOCK",fw_store,fwu->image_size);
	} else {
		CTP_DBG("%s: CMD_READ_CONFIG_BLOCK failed, rc=%d\n", __func__, retval);
	}

	CTP_DBG("%s: TRYING DUMP FW via ?CMD_READ_FW_BLOCK?\n", __func__);
	retval = fwu_read_blocks(fw_store, sizeof(fw_store)/fwu->block_size, CMD_READ_FW_BLOCK);
	if (fwu->image_size > 0) {
		CTP_DBG("%s: ?CMD_READ_FW_BLOCK? success, read %d bytes\n", __func__, fwu->image_size);
		dump_buff("FW_CMD_READ_FW_BLOCK",fw_store,fwu->image_size);
	} else {
		CTP_DBG("%s: ?CMD_READ_FW_BLOCK? failed, rc=%d\n", __func__, retval);
	}

	//restore
	fwu->image_size = img_size;

#else 

//	dump_config(UI_CONFIG_AREA);
...
static int fwu_start_dump(void)
{   
	int retval;
	struct f01_device_status f01_device_status;
    
	CTP_DBG("%s: Start of dump process\n", __func__);
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev, "Device is in %s mode\n", 
		f01_device_status.flash_prog == 1 ? "bootloader" : "UI");
	if (f01_device_status.flash_prog )
		dev_info(&fwu->rmi4_data->i2c_client->dev, "Device status %d\n",
				f01_device_status.status_code);
...
static void dump_configs(void)
{
	dump_config(PERM_CONFIG_AREA);
	dump_config(BL_CONFIG_AREA);
	dump_config(DISP_CONFIG_AREA);
	dump_config(UI_CONFIG_AREA);
}
...
static void dump_config(unsigned short cfg)
{
	...
	if (retval < 0) {
		CTP_DBG("%s: Failed to read config %d\n",
				__func__, cfg);
		return retval;
	} else {
		log_config();
...
static void log_config(void) 
{
	int i, size = fwu->config_size;
	u8 *buf = fwu->read_config_buf;
	char *id;
	
	printk("%s: dumping config %x area, size = %d\n", __func__, fwu->config_area, size );
...
static int fwu_start_write_config(void)
{
	...
	CTP_DBG("%s: Start of write config process\n", __func__);
	...
	CTP_DBG("%s: End of write config process\n", __func__);
...
int synaptics_fw_updater(unsigned char *fw_data)
{
	...
	CTP_DBG("synaptics_fw_updater: enter\n");
...
#if 1
		CTP_DBG("FW update: FORCE_UPGRADE\n");
		retval = fwu_start_reflash();
	    return retval;
#endif

	if (f01_device_status.flash_prog) {
		// force update if UI mode failed to load
		CTP_DBG("FW update: UI not loaded\n");
		retval = fwu_start_reflash();
	    return retval;
	} else
		if(image_configID > device_configID){	
            //LINE <touch panel> <DATE2013821> <start upgrade> zhangxiaofei
    		CTP_DBG("FW upgrade version\n");
	    	retval = fwu_start_reflash();
	        return retval;
	    }
	////////////////////////////////////////////////////////////////////////////
    CTP_DBG("FW update skipped\n");
...
int synaptics_fw_dumper()
{
	...
	CTP_DBG("synaptics_fw_dumper: enter\n");
    ...
    CTP_DBG("touch panel: device_configID=%d", device_configID);

	CTP_DBG("FW update: FORCE_UPGRADE\n");
	retval = fwu_start_dump();
    return retval;
}
EXPORT_SYMBOL(synaptics_fw_dumper);
...
static int fwu_read_blocks(unsigned char *block_ptr, unsigned short block_cnt,
		unsigned char command)
{
	int retval;
	unsigned char block_offset[] = {0, 0};
	...
	for (block_num = 0; block_num < block_cnt; block_num++) {
		...
		fwu->image_size += fwu->block_size;
	}

exit:
	fwu->rmi4_data->reset_device(fwu->rmi4_data);

	return fwu->image_size;
}
