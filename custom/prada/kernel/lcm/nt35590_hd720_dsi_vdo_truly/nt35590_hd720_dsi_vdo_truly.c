...
__setup("lcm.fbk_div=",fbk_div_setup);
/*
static int newlcm_setup(char *str)
{
    stock_driver = 0;
    printk("OPT %s: newlcm\n", __func__);
    return 0;
...
static struct opt_handler lcm_opts[] = {
    { "newlcm",       newlcm_setup },
    { "lcm.fbk_div=", fbk_div_setup }
};

#define LCM_OPT_SIZE sizeof lcm_opts / sizeof lcm_opts[0]

static void process_option(char *buf)
{
    int i;
    if (strlen(buf)==0)
        return;
    ...
    for (i=0; i<LCM_OPT_SIZE; i++)
    {
        char *opt = lcm_opts[i].option;
        int len = strlen(opt);
        if(strncmp(buf, opt, len) == 0)
        {
            (*lcm_opts[i].handler)(buf+len);
            return;
        }
    }
}
...
static void parse_cmdline()
{
    ...
    strncpy(buf, saved_command_line, COMMAND_LINE_SIZE);
...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		params->dsi.fbk_div = fbk_div;    // fref=26MHz, fvco=fref*(fbk_div+1)*2/(div1_real*div2_real)
}
