...
static int tpd_power(struct i2c_client *client, int on)
{
	...
	for(i = 0 ; i < 4; i++)
	{
		ret = tpd_i2c_write_byte_data(client, (fd_11.controlBase + 6 + i), tpd_res_10[i]); /* set touchscreen x/y resolution*/
		if (ret < 0)
        	{
	...
	for(i = 0 ; i < 4; i++)
	{
		ret = tpd_i2c_write_byte_data(client, (fd_11.controlBase + 6 + i), tpd_res_5[i]); /* set touchscreen x/y resolution*/
		if (ret < 0)
        	{
            		TPD_DMESG("Error touch can not set resolution. \n");
			return ret;
