...
u8 tpd_res_5[4]={(u8)CUSTOM_MAX_WIDTH, (u8)(CUSTOM_MAX_WIDTH>>8),\
			(u8)(CUSTOM_MAX_HEIGHT + TPD_BUTTON_HEIGH), (u8)((CUSTOM_MAX_HEIGHT + TPD_BUTTON_HEIGH)>>8)};
u8 tpd_res_10[4]={(u8)CUSTOM_MAX_WIDTH, (u8)(CUSTOM_MAX_WIDTH>>8),\
            ...
            for (i = 0; i < MAX_KEY_NUM; i++)
            {
              if((button & (0x01 << i))==1){
                  ...
                  if (FACTORY_BOOT == boot_mode || RECOVERY_BOOT == boot_mode)
                  {
                      ...
                      tpd_down(130,1320,50,SYNAPTICS_TOUCH_TRACK_IDS-1);
                  }
              ...
              if(FACTORY_BOOT == boot_mode || RECOVERY_BOOT == boot_mode)
                  {
                      ...
                      tpd_down(360,1320,50,SYNAPTICS_TOUCH_TRACK_IDS-1);
                  }
                  id = i;
              }
              if((button & (0x01 << i))==4){
              if (FACTORY_BOOT == boot_mode || RECOVERY_BOOT == boot_mode)
              {
                input_report_key(tpd->dev, touch_key_array[i], button & (0x01 << i));
                    ...
                    tpd_down(600,1320,50,SYNAPTICS_TOUCH_TRACK_IDS-1);
              }
              id = i;
              }
            }
...
static int tpd_sw_power(struct i2c_client *client, int on)
{
	...
	//send custom resolution
        if (ts->points_supported == 10)
	{
	    for(i = 0 ; i < 4; i++)
	    {
		retval = tpd_i2c_write_data(client, (ts->f11.ctrl_base + 6 + i), &tpd_res_10[i],sizeof(tpd_res_10[i])); /* set touchscreen x/y resolution*/
		if (retval < 0)
        	{
	...
	else
	{
	    for(i = 0 ; i < 4; i++)
	    {
		retval = tpd_i2c_write_data(client, (ts->f11.ctrl_base + 6 + i), &tpd_res_5[i],sizeof(tpd_res_5[i])); /* set touchscreen x/y resolution*/
		if (retval < 0)
        	{
            		TPD_DMESG("Error touch can not set resolution. \n");
			goto out;
