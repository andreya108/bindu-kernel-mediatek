...
u32 pinSet[3][8] = {
    ...
    {
        pinSet[1][IDX_PS_CMPDN+IDX_PS_ON] = GPIO_OUT_ONE;
        pinSet[1][IDX_PS_CMPDN+IDX_PS_OFF] = GPIO_OUT_ZERO;
    }
#else
...
//        if(mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN failed!! \n");}
	...
	{
		PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);
		PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_IMX111_MIPI_RAW \n");
	...
	else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX188_MIPI_RAW,currSensorName)))
	{
		PK_DBG("[%s]sensorIdx:%d \n",currSensorName,SensorIdx);
/*
#ifdef GPIO_CAMERA_AF_EN_PIN
                if(mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[[CAMERA SENSOR]  enable af en out 1 failed!! \n");}
#endif
...
*/
		if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D_SUB, VOL_1200,mode_name))//DVdd
		{
			 PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			 //return -EIO;
			 goto _kdCISModulePowerOn_exit_;
		...
		if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800/*VOL_2800*/,mode_name))//IOVdd
		{
			PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			//return -EIO;
			goto _kdCISModulePowerOn_exit_;
...
*/
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV9724_MIPI_RAW,currSensorName)))
	{
		PK_DBG("[%s]sensorIdx:%d \n",currSensorName,SensorIdx);
/*
#ifdef GPIO_CAMERA_AF_EN_PIN
...
*/
		if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D_SUB, VOL_1500,mode_name))//DVdd
		{
			 PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			 //return -EIO;
			 goto _kdCISModulePowerOn_exit_;
...
*/
	...
	else if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_OV12830_MIPI_RAW,currSensorName)) || 0 == strcmp(SENSOR_DRVNAME_OV12830TRULYB_MIPI_RAW,currSensorName)))
	{
		PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP0A28_MIPI_YUV,currSensorName)))
	{
		PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);
		PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_SP0A28_MIPI_YUV \n");

#if 0//def GPIO_CAMERA_AF_EN_PIN
                if(mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[[CAMERA SENSOR]  enable af en out 1 failed!! \n");}
#endif
	...
	{
		PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);
		PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_OV5648SUB_MIPI_RAW \n");
		...
		if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D_SUB, VOL_1500,mode_name))
		{
			 PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			 //return -EIO;
			 goto _kdCISModulePowerOn_exit_;
