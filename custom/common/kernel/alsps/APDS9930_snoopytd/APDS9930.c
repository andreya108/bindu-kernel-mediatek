...
/*  update lux convert algo from avago FAE --20130817 molg1.lenovo begin*/
	...
	if ( (ch0data >= apds9930_als_res_tb[data->als_atime_index]) && (ch1data >= apds9930_als_res_tb[data->als_atime_index]) ){
	...
	if ( (ch1ch0_ratio >= 30) && (ch1ch0_ratio <= 45) ) {
		// sunglight
		luxValue = (ch0data*100)/((apds9930_als_integration_tb[data->als_atime_index]/100)*apds9930_als_again_tb[data->als_again_index]);
		//luxValue = (luxValue * APDS9930_OUTDOOR_GA)/100;	// kk 25-Jun-2014
		luxValue = (luxValue * APDS9930_OUTDOOR_GA);	// kk 25-Jun-2014
...
/*lenovo-sw molg1 add for snoopytd 20131010 begin*/
...
/*lenovo-sw molg1 add for snoopytd 20131010 end*/
				}
				else{	
						databuf[0] = APDS9930_CMM_INT_LOW_THD_LOW;	
...
/*lenovo-sw molg1 add for snoopytd ps cali 20131104 begin*/		
		 if( data_cali<=100)
		{
			ps_data_cali->close =270;
			ps_data_cali->far_away =175;
		}
		 else if( data_cali<=168)
		{
			ps_data_cali->close =337;
			ps_data_cali->far_away =232;
		}
		else if( data_cali<=300)
		{
			ps_data_cali->close =data_cali *2;
			ps_data_cali->far_away =data_cali *16/10;
		}
...
/*lenovo-sw molg1 add for snoopytd ps cali 20131104 end*/
