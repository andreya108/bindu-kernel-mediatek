...
void GC0328NightMode(kal_bool bEnable)
{
	...
	{	
		...
		if(GC0328_MPEG4_encode_mode == KAL_TRUE)
			...
			GC0328_write_cmos_sensor(0x33, 0x20);//30_dj
		GC0328_write_cmos_sensor(0xfe, 0x00);
	...
	else 
	{
		...
		if(GC0328_MPEG4_encode_mode == KAL_TRUE)
			...
			GC0328_write_cmos_sensor(0x33, 0x20);//20_dj
		GC0328_write_cmos_sensor(0xfe, 0x00);
...
void GC0328_Sensor_Init(void)
{
	...
	GC0328_write_cmos_sensor(0xcb , 0x78);//add  
	GC0328_write_cmos_sensor(0xcc , 0x80);//add  
	GC0328_write_cmos_sensor(0xcd , 0x80);//add  
	...
	GC0328_write_cmos_sensor(0x17 , 0x14);//14
	...
	GC0328_write_cmos_sensor(0xd2 , 0x21);//cr red
	GC0328_write_cmos_sensor(0xd3 , 0x38);//add
	...
	GC0328_write_cmos_sensor(0x13 , 0x75);//80_dj   
...
BOOL GC0328_set_param_banding(UINT16 para)
{
	switch (para)
	{
		...
		case AE_FLICKER_MODE_60HZ:
			...
			GC0328_write_cmos_sensor(0x31, 0x03);   //exp level 0  7.05fps
			GC0328_write_cmos_sensor(0x32, 0xa8); 
			GC0328_SET_PAGE0;
		break;
		default:
		return FALSE;
