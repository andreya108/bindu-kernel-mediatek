...
MSDK_SCENARIO_ID_ENUM imx135CurrentScenarioId = MSDK_SCENARIO_ID_CAMERA_PREVIEW;
...
#define IMX135_OTP_SUPPORT
#ifdef IMX135_OTP_SUPPORT
...
static kal_bool IMX135MIPI_ReadOtp(int tempbank,kal_uint16 address,kal_uint8* iBuffer,int buffersize)
{
	kal_uint16 reVal;
	kal_uint16 reg;
	int i = 0;
	...
	{
		i=0;
		IMX135MIPI_write_cmos_sensor(0x3b02, tempbank);
		IMX135MIPI_write_cmos_sensor(0x3b00, 0x01);
		msleep(100);
		reg = IMX135MIPI_read_cmos_sensor(0x3b01);
		if ((reg & 0x01) ==0x01)
		{
			SENSORDB("IMX135MIPI_ReadOtp(), 读OTP 成功!");
		}
		else
		{
			SENSORDB("IMX135MIPI_ReadOtp(), 读OTP 失败!");
			return KAL_FALSE;
		}

		while(i<64)
		{
			reVal = IMX135MIPI_read_cmos_sensor(address+i);
			*(iBuffer+k) = reVal;
			i++;
			k++;
			if (k>=buffersize)
...
static void UpdateAWBforIMX135(kal_uint16 RoverG_dec,kal_uint16 BoverG_dec,kal_uint16 GboverGr_dec)
{
	...
	RoverG_dec_base = 607;//the typcical value
	BoverG_dec_base = 601;//the typcical value
	GboverGr_dec_base = 1025;//the typcical value
	...
	IMX135MIPI_write_cmos_sensor(0x020F,G_test_L8);
	IMX135MIPI_write_cmos_sensor(0x0211,R_test_L8);
	IMX135MIPI_write_cmos_sensor(0x0213,B_test_L8);
	IMX135MIPI_write_cmos_sensor(0x0215,G_test_L8);
}

//read module information and awb data
static kal_bool IMX135MIPI_ReadAWBFromOtp()
{
	kal_uint16 awbGroupbank[] = {0x02,0x01};//the two fist group for AWB data
	kal_uint16 address = 0x3b04;
	kal_uint16 MID = 0x01;
	kal_uint16 LensID = 0x03;
	kal_uint16 VcmID = 0x31;
	kal_uint16 r,g,b;
	int i;
	kal_uint8 Temp[42]={0};
	kal_uint16 temp3;
	...
	SENSORDB("IMX135MIPI_ReadAWBFromOtp() ++");

	//1.check valid group
	for(i=0;i<2;i++)
	{
		//select OTP page address for read
		IMX135MIPI_write_cmos_sensor(0x3b02,awbGroupbank[i]);//select page
		//turn on OTP read mode
		IMX135MIPI_write_cmos_sensor(0x3b00,0x01);//read mode:0x3b00 = 0x01 
		//check status(bit0:0x01 read ready)
		temp3 = IMX135MIPI_read_cmos_sensor(0x3b01);//Status check : 0x3B01 = 0x01 (bit0 1:read ready)
		if ((temp3&0x01) ==0x01)
		{
			SENSORDB("IMX135MIPI_ReadAWBFromOtp(), 读OTP 成功!\n");//显示
		}
		else
		{
			SENSORDB("IMX135MIPI_ReadAWBFromOtp(), 读OTP 失败!");
			return KAL_FALSE;
		}
		
		//0x3b04-0x3b43 are the register address area for each page
		//check program flag by read ox3b04
		temp3 = IMX135MIPI_read_cmos_sensor(0x3b04);
	...
	if (index == -1)
	{
		SENSORDB("IMX135MIPI_ReadAWBFromOtp(), OTP 中没有发现LSC 数据。");
		return KAL_FALSE;
	}
	
	//2.read data from the valid group
	int tempbank = awbGroupbank[index];
	if(KAL_FALSE == IMX135MIPI_ReadOtp(tempbank,address,Temp,42))
	{
		SENSORDB("IMX135MIPI_ReadAWBFromOtp(), 读取Otp 数据时，I2C 通信出现错误。");
		return KAL_FALSE;
	}
	...
	if (Temp[1] != (sum%256))
	{
		SENSORDB("IMX135MIPI_ReadAWBFromOtp(), OTP 中发现AWB 数据有误。");
		return KAL_FALSE;
	}
	...
	{
		SENSORDB("IMX135MIPI_ReadAWBFromOtp(), 模组ID、镜头ID 或者VCM ID 出错。");
		return KAL_FALSE;
	}
	#endif
...
//read lsc data
static kal_bool IMX135MIPI_ReadLSCFromOtp()
{
	kal_uint16 LscGroupbank[] = {0x09,0x03};//the two fist group for lsc data
	kal_uint16 address = 0x3b04;
	int i;
	kal_uint8 Temp[384]={0};
	kal_uint16 temp3;
	int index = -1;
	
	SENSORDB("IMX135MIPI_ReadLSCFromOtp() ++\n");
	//1.check valid group
	for(i=0;i<2;i++)
	{
		//select OTP page address for read
		IMX135MIPI_write_cmos_sensor(0x3b02,LscGroupbank[i]);
		//turn on OTP read mode
		IMX135MIPI_write_cmos_sensor(0x3b00,0x01);
		//check status(bit0:0x01 read ready)
		temp3 = IMX135MIPI_read_cmos_sensor(0x3b01);
		
		if ((temp3&0x01) ==0x01)
		{
			SENSORDB("IMX135MIPI_ReadLSCFromOtp(), 读OTP 成功!\n");
		}
		else
		{
			SENSORDB("IMX135MIPI_ReadLSCFromOtp(), 读OTP 失败!\n");
			return KAL_FALSE;
		}
		//0x3b04-0x3b43 are the register address area for each page
		//check program flag by read ox3b04
	...
	if (index == -1)
	{
		SENSORDB("IMX135MIPI_ReadLSCFromOtp(), OTP 中没有发现LSC 数据。\n");
		return KAL_FALSE;
	}
	
	//2.read data from the valid group
	int tempbank = LscGroupbank[index];
	if(KAL_FALSE == IMX135MIPI_ReadOtp(tempbank,address,Temp,384))
	{
		SENSORDB("IMX135MIPI_ReadLSCFromOtp(), 读取Otp 数据时，I2C 通信出现错误。\n");
		return KAL_FALSE;
	}
	...
	if (Temp[1] != (sum%256))
	{
		SENSORDB("IMX135MIPI_ReadLSCFromOtp(), OTP 中发现LSC 数据有误。\n");
		return KAL_FALSE;
	}
	//4.write shading to sensor
	for(i=0;i<504;i++)
	{
		IMX135MIPI_write_cmos_sensor(0X4800+i, tempBuffer[i]);
		SENSORDB("OTP LSC: %x , %x \n",0X4800+i, tempBuffer[i]);
	}
...
static kal_bool OnReadOtpIMX135(void)
{
	SENSORDB("OnReadOtpIMX135() --\n");

	// off lsc
	IMX135MIPI_write_cmos_sensor(0x4500,0x0);
	IMX135MIPI_write_cmos_sensor(0x0700,0x0);
	IMX135MIPI_write_cmos_sensor(0x3a63,0x0);
	if (KAL_FALSE == IMX135MIPI_ReadLSCFromOtp())
	{
		return KAL_FALSE;
	}
	if (KAL_FALSE == IMX135MIPI_ReadAWBFromOtp())
	{
		return KAL_FALSE;
	}
...
static void IMX135MIPI_Sensor_Init(void)
{
	...
	IMX135MIPI_write_cmos_sensor(0x0101, 0x03);//
...
UINT32 IMX135MIPIOpen(void)
{
...
#ifdef IMX135_OTP_SUPPORT
    {
    	...
    	if (ret) {
    		SENSORDB("call OnReadOtpIMX135() ok.\n"); 
    	} else {
    		SENSORDB("call OnReadOtpIMX135() bad.\n"); 
    	}
    }
#endif
...
{
		spin_lock(&imx135_drv_lock);	
		imx135CurrentScenarioId = ScenarioId;
		spin_unlock(&imx135_drv_lock);
    switch (ScenarioId)
    {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_PERIOD:
        		switch(imx135CurrentScenarioId)
        		{
        			case MSDK_SCENARIO_ID_CAMERA_ZSD:
        		    case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
