...
static struct LCM_setting_table lcm_initialization_setting[] =
{
	...
	{0x00, 1 , {0x00}},
	{0xE1, 16 ,{0x02,0x07,0x0C,0x0C,0x05,0x0B,0x09,0x09,0x02,0x04,0x11,0x27,0x14,0x1B,0x16,0x12}},

	{0x00, 1 , {0x00}},
	{0xE2, 16 ,{0x02,0x08,0x0C,0x0C,0x05,0x0B,0x09,0x09,0x02,0x04,0x11,0x27,0x14,0x1B,0x16,0x12}},
...
#ifdef BUILD_LK
	printf("lanhong lcd id: 0x%08x\n", id);
#else
	printk("lanhong lcd id: 0x%08x\n", id);
#endif        

	lcd_id =  mt_get_gpio_in(GPIO168);

	#ifdef BUILD_LK
		printf("lanhong:%s, LK otm9608a debug: otm9608a id = 0x%08x\n", __func__, lcd_id);
	#else
		printk("lanhong:%s, kernel otm9608a horse debug: otm9608a id = 0x%08x\n", __func__, lcd_id);
	#endif
	
	 if((LCM_ID == id) &&( lcd_id == 0))
	 {
	 	return 1;	
	 }
	 else
