...
/*Begin lenovo-sw, wengjun1, 20130515, add for slide touch */

static struct hrtimer tpd_resume_timer;
#define TIMER_MS_TO_NS(x) (x * 1000 * 1000)
void tpd_set_usb_flag_handler(void);
static DECLARE_WORK(tpd_set_usb_flag_work, tpd_set_usb_flag_handler);

#if defined(LENOVO_CTP_SLIDE_WAKEUP) || defined(DOUBLE_CLICK_TOUCH)
...
#endif
/*End lenovo-sw, wengjun1, 20130515, add for slide touch */
...
*/
		...
		printk("[wj]Reg 0xb8 value is 0x%02x.\n", val);
		if(val == 0x1 || val == 0x2)
		{
			suspend_flag = 0;
			//ret = i2c_write_bytes(i2c_client_point, 0x814b, &data, 1);
			//remove by lixh10 for unlock slowly!!!
			/*
...
unsigned char ft5x0x_read_ID_ver(void)
{
	unsigned char ver;
...
EXPORT_SYMBOL(ft5x0x_read_ID_ver);
...
{
	#include "Lenovo_Snoopy_PVT_0400000d_20130827_app.i"
};

static unsigned char CTPM_FW1[]=
{
	#include "Lenovo_Snoopy_PVT_01000006_20130826_app.i"
};
...
static int fts_ctpm_fw_upgrade_with_i_file(void)
{
   ...
   unsigned char version=0;
	...
	unsigned char type = ft5x0x_read_ID_ver();
	...
	if(type == 0x01)
	{
		...
		pbt_buf = CTPM_FW1;
  		...
  		i_ret =  fts_ctpm_fw_upgrade_1(pbt_buf,sizeof(CTPM_FW1));
	}
	else
	{
		if(version == 0x0d)
...
#ifdef CONFIG_TOUCH_PEF_TEST
	mt_set_gpio_out(GPIO34, GPIO_OUT_ONE);	
#endif
 }
...
struct delayed_work cpu_holder_handler_work;
...
static void boost_cpu_work_func(struct work_struct *work)
{
	...
	last_boost_time = cur;
	if (tpd_hold_cpu_ms) { //set tpd_hold_cpu_ms to disable 4 cores holding
		hp_based_cpu_num(tpd_boost_cpu_cores);
		schedule_delayed_work(&cpu_holder_handler_work, msecs_to_jiffies(tpd_hold_cpu_ms));
	}
}
...
int set_tp_protect(bool flag)
{
	...
	if(flag)	
	{
		printk("[wj]set flag as true.\n");
		usb_cable_flag = 1;
		ft5x0x_write_reg(0x8b, 0x01);
	}
	else
	{
		printk("[wj]set flag as false.\n");
		usb_cable_flag = 0;
		ft5x0x_write_reg(0x8b, 0x00);
	}
}
...
#ifdef CONFIG_TOUCH_PEF_TEST
	 mt_set_gpio_mode(GPIO34, GPIO_MODE_00);
	 mt_set_gpio_dir(GPIO34, GPIO_DIR_OUT);
	 mt_set_gpio_out(GPIO34, GPIO_OUT_ZERO);	
#endif
...
{
	int ret;
	int LEMDA_X = 0, LEMDA_Y = 0;
	u8 data_x,data_y = 0;

	data_x = ((( max_x - 1)*256)/(13*64 - 1 - (2 * LEMDA_X)))/2;
	data_y = ((( max_y - 1)*256)/(23*64 - 1 - (2 * LEMDA_Y)))/2;

	printk("[wj]the max_x = %d, max_y = %d, data_x = %d, data_y = %d .\n", max_x, max_y, data_x, data_y);
	ret = i2c_smbus_write_i2c_block_data(i2c_client, 0xb5, 1, &data_x);
	ret |= i2c_smbus_write_i2c_block_data(i2c_client, 0xb6, 1, &data_y);
	if (ret < 0)
...
void tpd_set_usb_flag_handler(void)
{
	printk("[wj]tpd tpd_set_usb_flag_handler begin.\n");
	set_tp_protect(usb_cable_flag);
}

enum hrtimer_restart tpd_resume_handler(struct hrtimer *timer)
{
	printk("[wj]tpd resume hrtimer begin.\n");
	tpd_halt = 0;
#ifdef SET_MAX_X_Y
	set_max_x_y(480, 800);
#endif
	suspend_flag = 0;	
	schedule_work(&tpd_set_usb_flag_work);
/*
	tpd_up(0,0,0);
	input_sync(tpd->dev);
*/
...
//		fts_ctpm_fw_upgrade_with_i_file();
 ...
 static int tpd_local_init(void)
 {
  ...
  TPD_DMESG("Focaltech FT5206 I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
   ...
   if(i2c_add_driver(&tpd_i2c_driver)!=0)
    ...
    /*lenovo-sw lixh10 add 201403208 for read fw-version begin*/
	get_tpd_info();
    /*lenovo-sw lixh10 add 20140328 for read fw-version end*/
 ...
 static void tpd_resume( struct early_suspend *h )
 {
    ...
    ktime = ktime_set(0, TIMER_MS_TO_NS(200));	
