...
#define GHOST_THRESHOLD_MAX 100
static unsigned long ghost_threshold = 0;
static unsigned long key_ms = 0;
static unsigned long touch_ms = 0;

static struct timeval te_key = {0};
static struct timeval te_touch = {0};

static int __init ghost_threshold_setup(char *str)
{
    unsigned long tmp = ghost_threshold;
    printk("OPT %s: %s\n", __func__, str);
    get_option(&str, &tmp);
    printk("OPT %s: tmp=%d\n", __func__, tmp);
    if (tmp <= GHOST_THRESHOLD_MAX)
        ghost_threshold = tmp;
    return 0;
}
early_param("tpd.ghost_threshold",ghost_threshold_setup);

unsigned long time_diff_ms(struct timeval *tstart, struct timeval *tend)
{
    return 1000 * (tend->tv_sec - tstart->tv_sec) + (tend->tv_usec - tstart->tv_usec) / 1000;
}

unsigned long time_from_ms(struct timeval *start)
{
    struct timeval te;
    do_gettimeofday(&te); // get current time
    return time_diff_ms(start, &te);
}

int time_is_0(struct timeval *te)
{
    return te->tv_sec == 0 && te->tv_usec == 0;
}

void time_set_0(struct timeval *te)
{
    te->tv_sec = 0;
    te->tv_usec = 0;
}
...
#endif//lenovo jixu add 20130415 end area touch
//    TPD_DMESG("--> tpd_work_func read data to clear interrupt!!\n");

    //clear interrupt bit
    retval = tpd_i2c_read_data(ts->client, ts->f01.data_base + 1, &status, 1);
    if (retval < 0)
    {
  //      TPD_DMESG("<-- tpd_work_func [retval=%d]\n", retval);
        return;
    }
    ...
    { /* key */
        ...
        time_set_0(&te_touch);
        if (time_is_0(&te_key)) do_gettimeofday(&te_key);
        key_ms = time_from_ms(&te_key);

//        TPD_DMESG("[%d] ===> hx read button!!!!, ts->f1a.data_base is 0x%08x ... \n", retval, ts->f1a.data_base);
        if (button)
        {
        ...
        {
            if (not_ghost(key_ms, "key   report"))
            {
                TPD_DMESG("key = %d, m=%ld\n", id, key_ms);
                input_report_key(tpd->dev, touch_key_array[id], 0);
            }
            time_set_0(&te_key);
        }
        if (not_ghost(key_ms, "key   sync"))
        {
            input_sync(tpd->dev);
        }
        return;
...
//    TPD_DMESG("status: %d\n",status);
    ...
    {
        time_set_0(&te_key);
        if (time_is_0(&te_touch)) do_gettimeofday(&te_touch);
        touch_ms = time_from_ms(&te_touch);
...
//                TPD_DMESG("finger addr=:%d!!\n",temp);
                ...
                Yw = ((data[3] >> 4) & 0x0F);
                XYsqr = Xw*Yw;
//                printk("[AA] %s data: %02X%02X%02X%02X%02X\n",__func__,data[0],data[1],data[2],data[3],data[4]);
//                printk("[JX] %s XYsqr=%d\n",__func__,XYsqr);
#endif//lenovo jixu add 20130415 end area touch
                ...
                if (not_ghost(touch_ms, "touch down"))
                {
                    TPD_DMESG("---> down %ld\n", touch_ms);
#if defined(LENOVO_AREA_TOUCH)//lenovo jixu modify 20130415 begin area touch
  ...
  //              TPD_DMESG("finger index=%d\n",(i+1));
        ...
        {
            if (not_ghost(touch_ms, "touch up"))
            {
                TPD_DMESG("---> up %ld\n", touch_ms);
                tpd_up(0, 0);
            }
        ...
        if (not_ghost(touch_ms, "touch sync"))
        {
            input_sync(tpd->dev);
        }
