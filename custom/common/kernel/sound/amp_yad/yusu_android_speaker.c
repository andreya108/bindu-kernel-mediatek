...
#if defined(LENOVO_NXP_SMARTPA_SUPPORT)
#define GPIO_SPEAKER_EN_PIN         (GPIO76 | 0x80000000)
#define GPIO_ANALOG_SWITCH_PIN      (GPIO211 | 0x80000000)
static bool gsk_boot_complete=false;
#endif
/*****************************************************************************
*                         D A T A      T Y P E S
******************************************************************************
...
#if defined(LENOVO_NXP_SMARTPA_SUPPORT)
   mt_set_gpio_mode(GPIO_ANALOG_SWITCH_PIN,GPIO_MODE_00);  // gpio mode
   mt_set_gpio_dir(GPIO_ANALOG_SWITCH_PIN,GPIO_DIR_OUT); // output
   mt_set_gpio_pull_enable(GPIO_ANALOG_SWITCH_PIN,GPIO_PULL_ENABLE);
   mt_set_gpio_out(GPIO_ANALOG_SWITCH_PIN,GPIO_OUT_ONE); // high
#endif

/* lenovo-sw zhouwl, 2013-04-16, add for 3-mic placement */
#if defined(LENOVO_THREE_MIC_SUPPORT)
	mt_set_gpio_mode(GPIO_MIC3_SWITCH_PIN,0);  
	mt_set_gpio_pull_enable(GPIO_MIC3_SWITCH_PIN, 1);
	mt_set_gpio_dir(GPIO_MIC3_SWITCH_PIN,1);
	mt_set_gpio_out(GPIO_MIC3_SWITCH_PIN,1); //gpio116, output, high, mic3
#endif	
/* lenovo-sw zhouwl, 2013-04-16, add for 3-mic placement */
...
#if defined(LENOVO_NXP_SMARTPA_SUPPORT)
    mt_set_gpio_dir(GPIO_ANALOG_SWITCH_PIN,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_ANALOG_SWITCH_PIN,GPIO_OUT_ZERO); // low
#endif
...
void Sound_Speaker_Turnoff(int channel)
{
    ...
    mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ZERO); // low
#if defined(LENOVO_NXP_SMARTPA_SUPPORT)
    if(gsk_boot_complete == false) 
    {
  	 gsk_on = false;
        gsk_boot_complete = true;
        return;
    }
...
/* lenovo-sw zhouwl, 2013-04-16, add for 3-mic placement */
#if defined(LENOVO_THREE_MIC_SUPPORT)
void Sound_SwitchMic_LoudSpkOn(void)
{
	mt_set_gpio_dir(GPIO_MIC3_SWITCH_PIN,1);
	mt_set_gpio_out(GPIO_MIC3_SWITCH_PIN,0); //gpio116, output, low, mic2
	printk("zwl, mic gpio116 = 0, mic2\n");
}

void Sound_SwitchMic_LoudSpkOff(void)
{
	mt_set_gpio_dir(GPIO_MIC3_SWITCH_PIN,1);
	mt_set_gpio_out(GPIO_MIC3_SWITCH_PIN,1); //gpio116, output, high, mic3
	printk("zwl, mic gpio116 = 1, mic3\n");
	msleep(SPK_WARM_UP_TIME);
}
#endif	
/* lenovo-sw zhouwl, 2013-04-16, add for 3-mic placement */
